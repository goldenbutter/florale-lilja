...
| Visitor badge URL pattern (locked: `left_color=%23333333` + `left_text=Views`) | ✅ commented | ✅ rendered | classic: `<img>` HTML-commented inside `<!-- View counter hidden ... -->` on all 5 pages with footers (404 has no footer); premium: rendered on same 5 pages with `page_id=demo-flowershop-premium.ibithun.com` |
...
| `og:image` URLs are absolute (https://...) | ✅ | ✅ | every og:image and twitter:image points at `https://demo-flowershop-{tier}.ibithun.com/...` |
...
| Visitor-badge rendered (uncommented) in every footer (5 pages — 404 has no footer) | ✅ | `grep -c "visitor-badge"` returns 1 hit on each of 5 pages with footers (index, bouquetter, om-oss, galleri, kontakt) — uncommented img tag rendering with `page_id=demo-flowershop-premium.ibithun.com` |
...
Live URLs are now `demo-flowershop-{classic,premium}.ibithun.com` (Vercel projects deployed 2026-05-05). The original audit was offline DOM inspection via Playwright `document.querySelectorAll('meta[property^="og:"]')`. https://www.opengraph.xyz/ round-trip can now be performed against the live URLs.
...
| Dedicated `og-*.jpg` variants <200 KB at 1200×630 (per family rule; bakery has these) | ✅ closed `880fef6` | Generated 5 files per tier via ffmpeg from existing content imagery: `og-preview.jpg` (home + 404, 71 KB), `og-bouquetter.jpg` (73 KB, portrait source padded with `#3A5A40`), `og-omoss.jpg` (189 KB), `og-galleri.jpg` (126 KB), `og-kontakt.jpg` (128 KB). All 12 HTML pages updated to reference the new files at 1200/630. |
| `og:image:width/height` meta corrected during audit (was lying with 1600×900; now 1376×768 landscape / 896×1200 portrait) | ✅ fixed → ✅ superseded `880fef6` | Now all 1200/630 referencing the dedicated `og-*.jpg` variants. |
| `bouquetter.html` OG image is portrait (`bouquet-romantic-pink.jpg` 896×1200) — bad ratio for social previews | ✅ closed `880fef6` | Swapped to dedicated `og-bouquetter.jpg` 1200×630 (sourced from `bouquet-summer.jpg` padded to landscape with brand deep-green `#3A5A40`). Alt text updated to match new content. |
| Classic mobile-menu lang-toggle markup present on only 2/5 footer pages | ✅ closed `880fef6` | Added commented-out EN toggle markup to nav-right + mobile-menu of `om-oss.html`, `galleri.html`, `kontakt.html`, `404.html` (4 pages × 2 locations) plus mobile-menu of `bouquetter.html` (1 location). All 6 classic pages now show 2 lang-toggle occurrences each. Markup verbatim from `index.html`. |
...
| Live URLs deployed | ✅ closed 2026-05-05 | `demo-flowershop-classic.ibithun.com` + `demo-flowershop-premium.ibithun.com` live. Vercel project names also `demo-flowershop-{classic,premium}` (English domain word, NOT `demo-blomsterbutikk-*` as originally specced — Bithun chose `flowershop` for the live URL while keeping local folder names `demo-blomsterbutikk-*`). |
...
| OG image strategy: 5 dedicated `og-*.jpg` files per tier at 1200×630 (71–189 KB each). Bakery uses 4 (`og-preview` + 3 page-specific); blomster adds `og-kontakt.jpg` because it has a dedicated kontakt page. | Closed `880fef6` — was drift, now matches family pattern. |
| Classic mobile-menu lang-toggle markup present on every nav + every mobile-menu of all 6 pages. Matches bakery. | Closed `880fef6` — was drift, now full parity. |
