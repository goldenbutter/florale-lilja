...
const translations = {
  no: {
    ...
    'hero.headline': 'Bundet for hånd, <em class="hero-rotator" data-rotator="1"></em>',
    'hero.rotator.0':'blomst for blomst',
    'hero.rotator.1':'med rolige hender',
    'hero.rotator.2':'i sesongens farger',
    'hero.rotator.3':'siden 2014',
  ...
  en: {
    ...
    'hero.headline': 'Hand-tied, <em class="hero-rotator" data-rotator="1"></em>',
    'hero.rotator.0':'stem by stem',
    'hero.rotator.1':'with quiet care',
    'hero.rotator.2':'in season colour',
    'hero.rotator.3':'since 2014',
...
// === LANGUAGE MANAGEMENT ===
let currentLang = localStorage.getItem('fl-lang') || 'no';
...
function applyTranslations() {
  document.querySelectorAll('[data-i18n]').forEach(el => {
    ...
    } else if (val.includes('<em') || val.includes('<span') || val.includes('<br')) {
      // Allow inline markup (used by hero.headline for the typewriter rotator slot).
      el.innerHTML = val;
    } else {
      el.textContent = val;
    }
...
function toggleLanguage() {
  ...
  applyTranslations();
  // Restart the hero typewriter chain so the rotator phrases swap to the
  // new language without leaving a half-typed Norwegian word on screen.
...
function initNav() {
  ...
  if (hamburger && mobileMenu) {
    // Premium tier: 240px liquid-glass floating card. No body-scroll lock.
    // Close on outside-click / Escape / link-click. Lang-toggle clicks do NOT close.
    function closeMenu() {
...
// Inspired by bilverksted's `revealHero` (CSS+JS pattern). The fixed prefix
// "Bundet for hånd, " (NO) / "Hand-tied, " (EN) never moves; only the
// <em class="hero-rotator" data-rotator="1"></em> slot types itself out,
// holds, deletes, and types the next phrase. Single timer chain so a
// language toggle restarts cleanly. Min-width is reserved at the widest
// phrase's measured size so the H1 doesn't reflow horizontally as the
// phrase length changes — that prevents the EN headline from wrapping to
// a second line on narrow viewports while a longer phrase is on screen.
let heroRotatorTimer = null;
function revealHero() {
  if (heroRotatorTimer) {
    clearTimeout(heroRotatorTimer);
    heroRotatorTimer = null;
  }

  const slot = document.querySelector('.hero-content .hero-rotator[data-rotator="1"]');
  if (!slot) return;

  const phrases = [];
  for (let i = 0; i < 8; i++) {
    const key = 'hero.rotator.' + i;
    const val = (translations[currentLang] && translations[currentLang][key]) ||
                (translations['no'] && translations['no'][key]);
    if (val) phrases.push(val); else break;
  }
  if (phrases.length === 0) return;

  // Off-screen probe to measure widest phrase so we can reserve horizontal
  // space — keeps the H1 single-line on narrow viewports.
  ...
  if (widest > 0) slot.style.minWidth = widest + 'px';

  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduced) {
    slot.textContent = phrases[0];
    slot.classList.remove('is-typing', 'is-deleting');
    return;
  }

  const TYPE_MS = 75;     // per-character type speed
  const DELETE_MS = 40;   // per-character delete speed
  const HOLD_MS = 1800;   // pause once a phrase is fully typed
  const GAP_MS = 320;     // pause after delete before next phrase
  ...
  const tick = () => {
    ...
    if (mode === 'typing') {
      ...
      charIdx += 1;
      slot.textContent = phrase.slice(0, charIdx);
      if (charIdx >= phrase.length) {
        mode = 'holding';
        slot.classList.remove('is-typing');
        heroRotatorTimer = setTimeout(tick, HOLD_MS);
      } else {
        heroRotatorTimer = setTimeout(tick, TYPE_MS);
      }
    } else if (mode === 'holding') {
      mode = 'deleting';
      heroRotatorTimer = setTimeout(tick, 0);
    } else if (mode === 'deleting') {
      slot.classList.add('is-deleting');
      charIdx -= 1;
      slot.textContent = phrase.slice(0, Math.max(0, charIdx));
      if (charIdx <= 0) {
        mode = 'gap';
        slot.classList.remove('is-deleting');
        phraseIdx = (phraseIdx + 1) % phrases.length;
        charIdx = 0;
        heroRotatorTimer = setTimeout(tick, GAP_MS);
      } else {
        heroRotatorTimer = setTimeout(tick, DELETE_MS);
      }
    } else if (mode === 'gap') {
      mode = 'typing';
      tick();
...
// === PREMIUM TRANSLATION EXTENSIONS ===
Object.assign(translations.no, {
  'bloom.weekly': 'Ukens blomst',

  'gen.label':    'Sesong-generator',
  'gen.heading':  'Finn buketten din',
  'gen.sub':      'Velg anledning, fargestemning og budsjett — vi viser deg buketter som passer akkurat den dagen du har i tankene.',

  'gen.f.occasion':'Anledning',
  'gen.f.mood':    'Fargestemning',
  'gen.f.budget':  'Budsjett',

  'gen.opt.alle':    'Alle',
  'gen.opt.hverdag': 'Hverdag',

  'gen.mood.pastell':'Pastell',
  'gen.mood.hvit':   'Hvit & grønn',
  'gen.mood.dyp':    'Dypt & romantisk',
  'gen.mood.mark':   'Markblomster',

  'gen.bud.under600':'Under 600',
  'gen.bud.middle':  '600–1 000',
  'gen.bud.over1000':'Over 1 000',

  'gen.empty':       'Ingen buketter matcher disse filtrene akkurat nå — prøv å justere én av valgene.',
  'gen.count.one':   'bukett funnet',
  'gen.count.many':  'buketter funnet',
  'gen.cta':         'Bestill via Vipps',
});

Object.assign(translations.en, {
  'bloom.weekly': 'Bloom of the week',

  'gen.label':    'Seasonal generator',
  'gen.heading':  'Find your bouquet',
  'gen.sub':      'Pick an occasion, a colour mood and a budget — we will show the bouquets that fit the day you have in mind.',

  'gen.f.occasion':'Occasion',
  'gen.f.mood':    'Colour mood',
  'gen.f.budget':  'Budget',

  'gen.opt.alle':    'All',
  'gen.opt.hverdag': 'Everyday',

  'gen.mood.pastell':'Pastel',
  'gen.mood.hvit':   'White & green',
  'gen.mood.dyp':    'Deep & romantic',
  'gen.mood.mark':   'Wildflowers',

  'gen.bud.under600':'Under 600',
  'gen.bud.middle':  '600–1 000',
  'gen.bud.over1000':'Over 1 000',

  'gen.empty':       'No bouquets match these filters right now — try adjusting one of the choices.',
  'gen.count.one':   'bouquet found',
  'gen.count.many':  'bouquets found',
  'gen.cta':         'Order via Vipps',
});

// === BOUQUET DATA (premium generator) ===
const BOUQUETS = [
  { id:'klassisk-hvit',  titleKey:'b1.title', descKey:'b1.desc', altKey:'b1.alt', image:'bouquet-classic-white.jpg', price:695,  priceLabel:'695',       occasion:['hverdag','takk','fødsel'], mood:['hvit'],          latin:'Rosa · Ranunculus · Eucalyptus' },
  { id:'pastell-vaar',   titleKey:'b2.title', descKey:'b2.desc', altKey:'b2.alt', image:'bouquet-pastel-spring.jpg', price:595,  priceLabel:'595',       occasion:['hverdag','fødsel','takk'], mood:['pastell'],       latin:'Paeonia · Tulipa · Ranunculus' },
  { id:'romantisk-rosa', titleKey:'b3.title', descKey:'b3.desc', altKey:'b3.alt', image:'bouquet-romantic-pink.jpg', price:745,  priceLabel:'745',       occasion:['kjærlighet','bryllup'],    mood:['dyp','pastell'], latin:'Rosa · Paeonia · Dahlia',     badgeKey:'b3.badge' },
  { id:'sommerhilsen',   titleKey:'b4.title', descKey:'b4.desc', altKey:'b4.alt', image:'bouquet-summer.jpg',        price:495,  priceLabel:'495',       occasion:['takk','hverdag','fødsel'], mood:['mark'],          latin:'Centaurea · Leucanthemum · Lavandula' },
  { id:'baerebukett',    titleKey:'b5.title', descKey:'b5.desc', altKey:'b5.alt', image:'bouquet-funeral.jpg',       price:1450, priceLabel:'1 450',     occasion:['begravelse'],              mood:['hvit'],          latin:'Lilium · Rosa · Gypsophila' },
  { id:'bryllup',        titleKey:'b6.title', descKey:'b6.desc', altKey:'b6.alt', image:'bouquet-wedding.jpg',       price:1800, priceLabel:'fra 1 800', occasion:['bryllup'],                 mood:['hvit','pastell'],latin:'Paeonia · Rosa · Gypsophila' },
];

// === BOUQUET GENERATOR (premium signature) ===
function initBouquetGenerator() {
  ...
  const filters = { occasion: 'alle', mood: 'alle', budget: 'alle' };

  function inBudget(price, budget) {
    if (budget === 'alle')      return true;
    if (budget === 'under600')  return price < 600;
    if (budget === '600to1000') return price >= 600 && price <= 1000;
    if (budget === 'over1000')  return price > 1000;
    return true;
  }

  function matches(b) {
    if (filters.occasion !== 'alle' && !b.occasion.includes(filters.occasion)) return false;
    if (filters.mood     !== 'alle' && !b.mood.includes(filters.mood))         return false;
    if (!inBudget(b.price, filters.budget))                                    return false;
    return true;
  }
  ...
  function render() {
    const matched = BOUQUETS.filter(matches);

    resultsEl.innerHTML = matched.map(b => {
      const altText  = escapeHtml(t(b.altKey));
      const title    = escapeHtml(t(b.titleKey));
      const desc     = escapeHtml(t(b.descKey));
      const ctaLabel = escapeHtml(t('gen.cta'));
      const badgeHtml = b.badgeKey
        ? '<span class="bouquet-card-badge" data-i18n="' + b.badgeKey + '">' + escapeHtml(t(b.badgeKey)) + '</span>'
        : '';
      return [
        '<article class="bouquet-card">',
        badgeHtml,
        '<img class="bouquet-image" src="assets/images/' + b.image + '" alt="' + altText + '" data-i18n="' + b.altKey + '" loading="lazy" />',
        '<div class="bouquet-body">',
          '<h3 data-i18n="' + b.titleKey + '">' + title + '</h3>',
          '<span class="bloom-latin" style="opacity:1; transform:none; margin-bottom: 0.85rem;">' + escapeHtml(b.latin) + '</span>',
          '<div class="bouquet-price">',
            '<strong>' + escapeHtml(b.priceLabel) + '</strong>',
            '<span>NOK</span>',
          '</div>',
          '<p data-i18n="' + b.descKey + '">' + desc + '</p>',
          '<button type="button" class="btn btn-primary vipps-trigger" data-i18n="gen.cta">' + ctaLabel + '</button>',
        '</div>',
        '</article>'
      ].join('');
    }).join('');
    ...
    if (countEl) {
      const noun = matched.length === 1 ? t('gen.count.one') : t('gen.count.many');
      countEl.innerHTML = matched.length
        ? '<strong>' + matched.length + '</strong> ' + escapeHtml(noun)
        : '';
    }

    if (emptyEl) emptyEl.style.display = matched.length ? 'none' : 'block';

    const backdrop = document.getElementById('vipps-modal');
    resultsEl.querySelectorAll('.vipps-trigger').forEach(btn => {
      btn.addEventListener('click', e => {
        e.preventDefault();
        if (!backdrop) return;
        backdrop.classList.add('open');
  ...
  form.querySelectorAll('.filter-pills button').forEach(btn => {
    btn.addEventListener('click', () => {
      const filter = btn.dataset.filter;
      const value  = btn.dataset.value;
      filters[filter] = value;
      form.querySelectorAll('button[data-filter="' + filter + '"]').forEach(b => b.classList.remove('active'));
